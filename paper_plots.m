...
%% (part of Figure 2) Plot overall mean across all cell classes and mean response of one cell for each cell class
...
for nc = 1:3
    ...
    %mean_sig_su = nanmean(sig_adapsu,2);
    sig_ci_low = meanSigSu - std(unit_trials(:,~all(isnan(unit_trials))),0,2, 'omitnan'); %./sqrt(length(find(~isnan(unit_trials(1,:)))));
    sig_ci_high = meanSigSu + std(unit_trials(:,~all(isnan(unit_trials))),0,2, 'omitnan') %./sqrt(length(find(~isnan(unit_trials(1,:)))));
    ...
    hold on
    wf_ci_low = meanWf - std(spikes(:,:,unitNum(nc)),0,1, 'omitnan'); %./sqrt(length(find(~isnan(spikes(:,1,unitNum(nc))))));
    wf_ci_high =  meanWf + std(spikes(:,:,unitNum(nc)),0,1, 'omitnan'); %./sqrt(length(find(~isnan(spikes(:,1,unitNum(nc))))));
    ...
    filename = strcat('C:\Users\daumail\OneDrive - Vanderbilt\Documents\LGN_data_042021\single_units\inverted_power_channels\good_single_units_data_4bumps_more\new_peak_alignment_anal\su_peaks_03032020_corrected\plots\',strcat(sprintf('example_%s_unit_spike_wf_std', cellclass(nc))));
    saveas(gcf, strcat(filename, '.svg'));
