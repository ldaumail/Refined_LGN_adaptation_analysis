function [peakLocs, NoFiltMultiContMUA] = peakLocsMUATrialSelectionCRFs(channelsdir, filenames)
...
for i = channum
    ...
    for n = 1:length(contLims)
        ...
        for tr = 1:length(powerDE)
            ...
            if mean_wnd1_DE(tr) > mean(mua_bsl)+1.96*std(mua_bsl)/sqrt(length(mua_bsl))  && powerDE(tr) > mean(power0)+1.96*std(power0)/sqrt(length(power0)) %/sqrt(length(sua_bsl)) /sqrt(length(power0))
